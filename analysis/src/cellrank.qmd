...
```{python plot-macrostates-comp, results = "hide", fig.width = 8}
...
state_df
# Convert to DF
terminals = g.macrostates.map({k:v for k, v in state_map.items() if k in tstates}).astype("category")
...
terminals = terminals["terminal"].cat.remove_unused_categories()
_tmp = pd.Series(index = adata.obs.index)
_tmp[terminals.index] = terminals
terminals = _tmp.astype("category")
g.set_terminal_states(terminals)
```

```{python def-plot-terminals}
def plot_terminals(gppca, basis, figsize = None):
    """
    Plot terminal states for a GPPCA object on an embedding.

    Parameters
    ----------
    ggpca : cellrank.estimators.terminal_states._gpcca.GPPCA
    basis : str
        basis to use for plotting
    figsize : (float, float) | None
        width, height in inches of the resulting figure. If None, automatically
        determined from the number of terminal states.
    ...
    """
    adata = gppca.adata
    terminals = gppca.terminal_states
    unique_terminals = terminals.dropna().unique()
    # mandate order
    terms = [
        "cTEC", "Ciliated", "Tuft", "Skin", "Neuroendocrine", "Aire-stage", "Microfold"
    ]
    terms = [x for x in terms if x in unique_terminals]
    terms = terms + [x for x in unique_terminals if x not in terms]
    ncol = 7
    nrow = math.ceil(len(terms) / ncol)
    if figsize is None:
        figsize = (4, 3)
    fig, ax = plt.subplots(nrow, ncol, figsize = (ncol, nrow * 1.2))
    for i, ax in enumerate(fig.axes):
        ax.axis("off")
        if i < len(terms):
            ax = sc.pl.embedding(adata, basis = basis, ax = ax, show = False)
            term_idx = terminals.index[terminals == terms[i]]
            xy = adata[term_idx].obsm[basis]
            ax.scatter(xy[:, 0], xy[:, 1], s = 2, c = "red")
            ax.set_title(terms[i], fontsize = 8)
            ax.set_aspect(1)
    fig.tight_layout()
    return fig
...
```{python plot-terminals}
fig = plot_terminals(g, "UMAP_int")
fig.show()
fig.savefig(os.path.join(r.FIGS, "fig_fate_terminals.pdf"))
fig.savefig(os.path.join(r.FIGS, "fig_fate_terminals.png"), dpi = 300)
```
...
<!-- This chunk only runs for VARIANT == "manual" -->
```{python prune-terminals, include = VARIANT == "manual", eval = VARIANT == "manual"}
pruned_terminals = terminals.cat.remove_categories(["Ionocyte", "Goblet"])
g.set_terminal_states(pruned_terminals)
fig = plot_terminals(g, "UMAP_int")
fig.show()
fig.savefig(os.path.join(r.FIGS, "fig_fate_terminals_pruned.pdf"))
fig.savefig(os.path.join(r.FIGS, "fig_fate_terminals_pruned.png"), dpi = 300)
```
...
# Mark start cell
res["start"] = False
res.loc[start, "start"] = True
# Mark terminal cells
terminals.name = "terminal"
res = res.merge(terminals, left_index = True, right_index = True)

res = res.reset_index(names = "transcriptome")
res.to_csv("pipeline/cellrank_fps.csv", index = False, na_rep = "NA")
```
...
dat_long <- dat_long %>%
    ...
    mutate(start = as.logical(start))
dat_wide <- dat_long %>%
    pivot_wider(id_cols = transcriptome, names_from = "fate", names_prefix = "fp_", values_from = "fate_probability") %>%
    left_join(distinct(select(dat_long, -fate, -fate_probability)))
...
p_umap_dpt <- ggplot(dat_wide, aes(UMAP_int.1, UMAP_int.2, color = dpt_pseudotime)) +
    geom_point() +
    # Mark start cell
    geom_point(data = ~filter(., start), shape = 21, color = "white", fill = "red", size = 2) +
...
```{r fig_fate_jaccard1, fig.width = 2.2, fig.height = 2.8}
...
threshs <- seq(0.1, 0.3, by = 0.1)
fp_jc <- filter(fp_jc, thresh %in% threshs)
p_fp_jc1 <- filter(fp_jc, thresh == 0.1) %>%
...
p_fp_jc1
fig_pdf(FIGS, p_fp_jc1)
```

```{r fig_fate_jaccard2, fig.width = 6, fig.height = 2.8}
p_fp_jc2 <- p_fp_jc1 %+%
    filter(fp_jc)
p_fp_jc2
fig_pdf(FIGS, p_fp_jc2)
```

```{r tbls}
# save data tables
tbl_umap <- select(dat_wide, transcriptome, sample, start, terminal, UMAP_int.1, UMAP_int.2, label, dpt_pseudotime, starts_with("fp_"))
write.table(tbl_umap, file.path(FIGS, "tbl_umap.tsv"), sep = "\t", row.names = FALSE)
write.table(fp_jc, file.path(FIGS, "tbl_jaccard.tsv"), sep = "\t", row.names = FALSE)
```
